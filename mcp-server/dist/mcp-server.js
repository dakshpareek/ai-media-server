...
const healthMonitor = new ProwlarrHealthMonitor(PROWLARR_CONFIG.baseUrl, PROWLARR_CONFIG.apiKey);
const searchManager = new ProwlarrSearchManager(PROWLARR_CONFIG.baseUrl, PROWLARR_CONFIG.apiKey);
const intelligentSearchManager = new IntelligentSearchManager(PROWLARR_CONFIG.baseUrl, PROWLARR_CONFIG.apiKey);
// Store last search results for download selection
let lastSearchResults = [];
// qBittorrent client variable (initialized in main)
let qbittorrentClient;
// Start the server
async function main() {
    // Initialize qBittorrent client (moved inside main to prevent startup issues)
    qbittorrentClient = new QBittorrentClient(QBITTORRENT_CONFIG.baseUrl, QBITTORRENT_CONFIG.username, QBITTORRENT_CONFIG.password, QBITTORRENT_CONFIG.sessionTimeout);
    // Create the server
    ...
    if (isStdioMode) {
        ...
        const ALLOWED_ORIGINS = (process.env.ALLOWED_ORIGINS || 'http://localhost:4343').split(',');
            ...
            switch (name) {
                ...
                */
                case 'prowlarr_search': {
                    ...
                    const indexerIds = args?.indexerIds;
                ...
                case 'prowlarr_vpn_connect': {
                    const city = args?.city || 'australia';
                    const vpnManager = intelligentSearchManager.getVPNManager();
                ...
                case 'qbittorrent_add_torrent': {
                    const options = {
                        ...
                        tags: args?.tags,
                        paused: args?.paused,
                        skipChecking: args?.skipChecking,

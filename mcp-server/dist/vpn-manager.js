...
export class VPNManager {
    ...
    manualDisconnectIntent = false;
    DISCONNECT_DELAY_MS = 10 * 60 * 1000; // 10 minutes
    CONNECTION_TIMEOUT_MS = 60000; // 60 seconds for VPN connection command
    COMMAND_TIMEOUT_MS = 30000; // 30 seconds for general commands
    NORDVPN_CONTAINER_NAME = 'nordvpn_official';
    lastConnectTime = null; // Store timestamp of successful connect call
    constructor() {
        this.clearDisconnectTimer();
    }
    async execInNordVPNContainer(command, timeout = this.COMMAND_TIMEOUT_MS) {
    ...
    parseNordVPNStatusOutput(stdout) {
        ...
        return status; // Cast, knowing other fields will be added
    }
    async getVPNStatus() {
        let authenticated;
        let needsLogin;
    ...
    async applyPostConnectionSettings() {
        ...
        try {
            ...
            if (localNetworkEnv) {
                ...
                catch (error) { /* console.warn(`VPNManager: Failed to whitelist ${localNetworkEnv} (post-connect): ${error.message}`); */ }
            }
            else {
    ...
    async connectVPN(preferredCity = '') {
...
//# sourceMappingURL=vpn-manager.js.map